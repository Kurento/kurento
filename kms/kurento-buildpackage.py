...
def apt_cache_update():
    ...
    try:
        ...
        print("[kbp.apt_cache_update] ERROR:", err)
        exit(1)
    APT_CACHE.open()
...
def apt_cache_commit():
    ...
    try:
        ...
        print("[kbp.apt_cache_commit] ERROR:", err)
        exit(1)
    APT_CACHE.open()
    root_privileges_drop()
...
def get_pkgversion_to_install(pkg, req_version, req_commit):
    def check_req_version(version_str, req_version):
        print("[kbp.get_pkgversion_to_install] Test '{} {} {}'".format(
    ...
    print("[kbp.get_version_to_install]"
...
def check_deb_dependency_installed(dep_alts):
    print("[kbp.check_deb_dependency_installed]"
        ...
        if APT_CACHE.has_key(dep_name):
            ...
            if pkg.is_installed:
                if pkgversion == pkg.installed:
                    ...
                    print("[kbp.check_deb_dependency_installed] WARNING:"
                          " Dependency '{}' is installed via package '{}'"
...
def get_version():
    print("[kbp.get_version] Run 'kurento_get_version.sh'")
    try:
        cmd_out = subprocess.check_output(["kurento_get_version.sh"]).strip()
    except subprocess.CalledProcessError:
        print("[kbp.get_version] ERROR: Running 'kurento_get_version.sh'")
        return None
    else:
        print("[kbp.get_version] Found version:", cmd_out)
...
def request_http(url, cert_path, key_path, file_path=None):
    if file_path is None:
        print("[kbp.request_http] Run request,"
        ...
        try:
            ...
            print("[kbp.request_http] ERROR: Running request:", err)
            exit(1)
        else:
            print("[kbp.request_http] DONE: Running request:\n", res.text)
    else:
        curl_cmd = ("curl --fail --insecure --key " + key_path
                    + " --cert " + cert_path
                    + " -X POST \"" + url + "\""
                    + " --data-binary @" + file_path)
        print("[kbp.request_http] Run command:", curl_cmd)
        try:
            curl_text = subprocess.check_output(curl_cmd, shell=True).strip()
        except subprocess.CalledProcessError:
            print("[kbp.request_http] ERROR: Running 'curl'")
            exit(1)
        else:
            print("[kbp.request_http] DONE: Running 'curl':\n", curl_text)
...
#     except IOError as err:
#         print("[kbp.request_http] ERROR:"
#               " Opening file '{}', error: {}".format(file_path, err.strerror))
#         exit(1)
#     else:
#         print("[kbp.request_http] File opened:", file_path)
...
def install_build_dependencies():
    ...
    print("[kbp.install_build_dependencies]"
        ...
        if not check_deb_dependency_installed(dep_alts):
            if check_deb_dependency_installable(dep_alts):
                ...
                if not install_dependency(dep_alts):
                    print("[kbp.install_build_dependencies] ERROR:"
...
def generate_debian_package(args, buildconfig):
    ...
    print("[kbp.generate_debian_package] ({})"
    ...
    new_version = get_debian_version(dist)
    if new_version is None:
        print("[kbp.generate_debian_package] ({}) ERROR:"
    ...
    if (args.kurento_fork):
        print("[kbp.generate_debian_package] ({})"
            " Build a Kurento fork: run 'gbp buildpackage', jobs: {}".format(project_name, cpu_count))
        try:
            subprocess.check_call(
                ["gbp", "dch", "--ignore-branch", "--snapshot", "--git-author", "debian/"])
        except subprocess.CalledProcessError:
            print("[kbp.generate_debian_package] ({}) ERROR:"
                " Running 'gbp dch'".format(project_name))
            exit(1)

        try:
            subprocess.check_call(
                ["gbp", "buildpackage", "--git-ignore-branch", "--git-ignore-new", "-uc", "-us", "-j" + str(cpu_count)])
        except subprocess.CalledProcessError:
            print("[kbp.generate_debian_package] ({}) ERROR:"
                " Running 'gbp buildpackage'".format(project_name))
            exit(1)
    else:
        print("[kbp.generate_debian_package] ({})"
            " Build a Kurento repo: run 'dpkg-buildpackage', jobs: {}".format(project_name, cpu_count))
        try:
            subprocess.check_call(
...
def check_dependency_installed(dependency, debian_control_file):
    print("[kbp.check_dependency_installed]"
...
def compile_project(args):
    ...
    try:
        ...
        print("[kbp.compile_project] ({}) ERROR:"
    ...
    print("[kbp.compile_project] ({}) Done.".format(project_name))
...
def main():
    ...
    if os.geteuid() != 0:
        print("[kbp.main] ERROR: This program needs root privileges")
        exit(1)
    root_privileges_drop()
    ...
    parser.add_argument("--kurento_fork",
                        action="store_true",
                        help="Build packages for a Kurento external fork")
    ...
    if not args.no_apt_get_update:
        print("[kbp.main] Run 'apt-get update'")
        apt_cache_update()
