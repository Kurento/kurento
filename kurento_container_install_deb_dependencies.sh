...
DEBIAN_FRONTEND=noninteractive postpone -d -f apt-get update || echo "Error updating packages"
...
DEBIAN_FRONTEND=noninteractive postpone -d -f apt-get install --force-yes -y $pkgs || exit 1
