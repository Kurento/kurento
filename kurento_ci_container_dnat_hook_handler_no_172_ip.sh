...
if [ $event = 'start' ]; then
  ...
  if [ $? = 0 ]; then
    ...
    sudo $(realpath $(dirname "$0"))/kurento_ci_container_dnat_no_172_ip.sh $container $event $transport $ip >> dnat2.log
  fi
fi
...
if [ $event = 'destroy' ]; then
  ...
  if [ -f $container.id ]; then
    ...
    rm $container.id
    echo "Calling dnat script"
    sudo $(realpath $(dirname "$0"))/kurento_ci_container_dnat_no_172_ip.sh $container $event $transport >> dnat2destroy.log
  else
