  ...
  public class PingWatchdogSession {
    ...
    public void pingReceived(long interval) {
      ...
      if (pingInterval == -1) {
        ...
        log.info("Setting ping interval to {}" + " millis in session with transportId={}. "
            + "Connection is closed if a ping is not received in {}x{}={} millis", pingInterval,
            this.transportId, pingInterval, NUM_NO_PINGS_TO_CLOSE, NUM_NO_PINGS_TO_CLOSE
                * pingInterval);
      }
    ...
    private void activateSessionCloser() {
      ...
      lastTask =
          taskScheduler.schedule(closeSessionTask, new Date(System.currentTimeMillis()
