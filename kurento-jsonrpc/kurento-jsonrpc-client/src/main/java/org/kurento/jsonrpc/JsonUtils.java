  ...
  private static JsonElement extractJsonValueFromResponse(JsonElement result, Type type) {
    ...
    if (isPrimitiveClass(type) || isEnum(type)) {
      ...
      if (result instanceof JsonPrimitive) {
        ...
        throw new JsonRpcException("Json array '" + result + " cannot be converted to "
            + getTypeName(type));
...
class JsonRpcResponseDeserializer implements JsonDeserializer<Response<?>> {
  ...
  public Response<?> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    if (!jObject.has(JSON_RPC_PROPERTY)) {
      throw new JsonParseException("Invalid JsonRpc response lacking version '" + JSON_RPC_PROPERTY
          + "' field");
    }
    ...
    if (idAsJsonElement != null) {
      try {
        ...
        throw new JsonParseException("Invalid format in '" + ID_PROPERTY + "' field in request "
            + json);
      }
    }
    ...
    if (jObject.has(RESULT_PROPERTY)) {
      ...
      return new Response<>(id, (ResponseError) context.deserialize(jObject.get(ERROR_PROPERTY),
          ResponseError.class));
...
class JsonRpcRequestDeserializer implements JsonDeserializer<Request<?>> {
  ...
  public Request<?> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    if (!(json instanceof JsonObject)) {
      throw new JsonParseException("Invalid JsonRpc request showning JsonElement type "
    ...
    if (!jObject.has(METHOD_PROPERTY)) {
      throw new JsonParseException("Invalid JsonRpc request lacking '" + METHOD_PROPERTY
          + "' field");
    }
    ...
    return new Request<>(id, jObject.get(METHOD_PROPERTY).getAsString(), context.deserialize(
        jObject.get(PARAMS_PROPERTY), parameterizedType.getActualTypeArguments()[0]));
