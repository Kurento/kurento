    ...
    if (resultClass == String.class || resultClass == Boolean.class
      ...
      if (resultJsonObject.isJsonObject()) {
        ...
        if (properties.size() > 1) {
          ...
          log.warn("Converting a result with {} properties in a value"
              + " of type {}. Selecting propoerty '{}'", Integer.valueOf(properties.size()),
              resultClass, prop.getKey());
        ...
        if (array.size() > 1) {
          log.warn("Converting an array with {} elements in a value "
              + "of type {}. Selecting first element", Integer.valueOf(array.size()), resultClass);
