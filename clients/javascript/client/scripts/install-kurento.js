...
try {
  ...
  try {
    ...
    gitRef = execSync('git rev-parse --abbrev-ref HEAD', { encoding: 'utf8' }).trim();
  ...
  execSync(`git clone --depth 1 --filter=blob:none --sparse --branch ${gitRef} ${repoUrl} ${tempDir}`, { stdio: 'inherit' });
  execSync(`cd ${tempDir} && git sparse-checkout set clients/javascript/jsonrpc`, { stdio: 'inherit' });
