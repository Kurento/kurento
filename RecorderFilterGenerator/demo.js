...
function startRecording() {
	...
	function onOffer(offer) {
		...
		co(function*(){
			try {
				var kwsMedia = yield new KwsMedia(ws_uri);
				pipeline = yield kwsMedia.create("MediaPipeline");
				...
				var mirror = yield pipeline.create("GStreamerFilter", {command : "videoflip method=4"});
				...
				var pointerDetector = yield pipeline.create("PointerDetectorAdvFilter", {calibrationRegion : calibrationRegion});
				var buttonWindow = {
						id: "myWindow",
						height: 100,
						width: 100,
				...
				pointerDetector.on("WindowIn", function(event){
					...
					times++;
					if(times % 2){
						faceOverlay.setOverlayedImage(hat_uri, offsetXPercent, offsetYPercent, widthPercent, heightPercent);
					} else {
