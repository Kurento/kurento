...
enum { PROP_0, PROP_IMAGE_TO_OVERLAY, PROP_SHOW_DEBUG_INFO };

struct _KmsImageOverlayPrivate {
  cv::Mat cvImage, costume;
...
kms_image_overlay_load_image_to_overlay (KmsImageOverlay *self)
{
  ...
  imageAux = cv::imread (url, cv::IMREAD_UNCHANGED);
  if (!imageAux.empty ()) {
  ...
  imageAux = cv::imread (file_name, cv::IMREAD_UNCHANGED);
  if (imageAux.empty ()) {
...
static void
kms_image_overlay_set_property (GObject *object,
    guint property_id,
    const GValue *value,
    GParamSpec *pspec)
{
...
static void
kms_image_overlay_get_property (GObject *object,
    guint property_id,
    GValue *value,
    GParamSpec *pspec)
{
...
static void
dispose_queue_element (GstStructure *data)
{
