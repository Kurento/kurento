...
message(STATUS "Looking for ${name} version @PROJECT_VERSION@")
...
get_filename_component(CURRENT_FILE <#noparse>${CMAKE_CURRENT_LIST_FILE}</#noparse> NAME)

if(${name}_FOUND)
  message(STATUS "[<#noparse>${CURRENT_FILE}</#noparse>] Found: <#noparse>${</#noparse>${name}<#noparse>_LIBRARY}</#noparse>")
else()
  message(STATUS "[<#noparse>${CURRENT_FILE}</#noparse>] Not found")
endif()
