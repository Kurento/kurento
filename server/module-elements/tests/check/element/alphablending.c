...
GST_START_TEST (connection)
{
  ...
  gchar *padname1;
  GstElement *mixer = gst_element_factory_make ("alphablending", NULL);
  GstStructure *data, *data2;
  ...
  data = gst_structure_new ("data", "port", G_TYPE_INT, handlerId1,
      "z_order", G_TYPE_INT, 1, NULL);

  g_object_set (G_OBJECT (mixer), MASTER_PORT, data, NULL);

  data2 = gst_structure_new ("data",
      "port", G_TYPE_INT, handlerId2,
      "relative_x", G_TYPE_FLOAT, 0,
      "relative_y", G_TYPE_FLOAT, 0,
      "relative_width", G_TYPE_FLOAT, 0.5,
      "relative_height", G_TYPE_FLOAT, 0.5, "z_order", G_TYPE_INT, 2, NULL);

  if (data2 != NULL) {
    g_signal_emit_by_name (mixer, SET_PORT_PROPERTIES, data2);
    gst_structure_free (data2);
  }

  data2 = gst_structure_new ("data",
      "port", G_TYPE_INT, handlerId3,
      "relative_x", G_TYPE_FLOAT, 0.7,
      "relative_y", G_TYPE_FLOAT, 0.7,
      "relative_width", G_TYPE_FLOAT, 0.5,
      "relative_height", G_TYPE_FLOAT, 0.4, "z_order", G_TYPE_INT, 3, NULL);
 ...
 */
static Suite *
alphableding_suite (void)
{
  Suite *s = suite_create ("alphableding");
