...
public class RoomParticipant implements Closeable {
  ...
  public void receiveVideoFrom(RoomParticipant sender, String sdpOffer) {
    ...
    final String ipSdpAnswer = this.createSdpResponseForUser(sender, sdpOffer);
  ...
  private String createSdpResponseForUser(RoomParticipant sender, String sdpOffer) {
    ...
    log.debug("PARTICIPANT {}: Creating a sending endpoint to user {}", this.name, sender.getName());
  ...
  public void close() {
    ...
    for (final String remoteParticipantName : sendingEndpoints.keySet()) {
      ...
      log.debug("PARTICIPANT {}: Released incoming EP for {}", this.name, remoteParticipantName);

      final WebRtcEndpoint ep = this.sendingEndpoints.get(remoteParticipantName);
