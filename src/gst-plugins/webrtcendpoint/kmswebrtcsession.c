...
kms_webrtc_session_remote_sdp_add_ice_candidate (KmsWebrtcSession *
    self, KmsIceCandidate * candidate, gboolean error_mode)
{
  KmsSdpSession *sdp_sess = KMS_SDP_SESSION (self);
  guint8 index;
  const GstSDPMedia *media;
  const GstDebugLevel dbg = (error_mode ? GST_LEVEL_ERROR : GST_LEVEL_DEBUG);

  if (sdp_sess->remote_sdp == NULL) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Remote SDP hasn't been set yet",
  ...
  if (index >= gst_sdp_message_medias_len (sdp_sess->remote_sdp)) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Invalid media index %u in remote SDP",
    ...
    gboolean error_mode)
{
  ...
  for (i = 0; i < len; i++) {
    ...
    kms_webrtc_session_remote_sdp_add_ice_candidate (self, candidate, error_mode);
  }
}

static gboolean
kms_webrtc_session_agent_add_ice_candidate (KmsWebrtcSession * self,
    KmsIceCandidate * candidate, gboolean error_mode)
{
  KmsSdpSession *sdp_sess = KMS_SDP_SESSION (self);
  guint8 index;
  const GstSDPMedia *media;
  ...
  if (sdp_sess->local_sdp == NULL) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Local SDP hasn't been generated yet",
        kms_ice_candidate_get_candidate (candidate));
    return TRUE;
  }
  ...
  if (!self->gather_started) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): ICE Gathering hasn't been started yet",
        kms_ice_candidate_get_candidate (candidate));
  ...
  if (index >= gst_sdp_message_medias_len (sdp_sess->local_sdp)) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Invalid media index %u in local SDP",
        kms_ice_candidate_get_candidate (candidate), index);
    return FALSE;
  }
  ...
  if (media == NULL) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): No media in local SDP at index %u",
        kms_ice_candidate_get_candidate (candidate), index);
    return FALSE;
  }
  ...
  if (sdp_utils_media_is_inactive (media)) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Inactive media at index %u",
        kms_ice_candidate_get_candidate (candidate), index);
    return TRUE;
  }
  ...
  if (handler == NULL) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): No handler for media at index %u",
        kms_ice_candidate_get_candidate (candidate), index);
    return FALSE;
  }
  ...
  if (stream_id == NULL) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): No stream_id at index %u",
        kms_ice_candidate_get_candidate (candidate), index);
    return FALSE;
  }
  ...
  if (!kms_ice_base_agent_add_ice_candidate (self->agent, candidate,
      stream_id)) {
    GST_CAT_LEVEL_LOG (GST_CAT_DEFAULT, dbg, self,
        "Cannot add remote candidate (%s): Agent failed for stream_id '%s'",
        kms_ice_candidate_get_candidate (candidate), stream_id);
    return FALSE;
  }

  GST_DEBUG_OBJECT (self,
      "Added remote candidate (%s): stream_id '%s'",
      kms_ice_candidate_get_candidate (candidate), stream_id);
  return TRUE;
}
...
kms_webrtc_session_gather_candidates (KmsWebrtcSession * self)
{
  ...
  GST_TRACE_OBJECT (self, "Gather candidates");
  ...
  if (ret) {
    ...
    // error_mode: true, because at this point the remote SDP should have been
    // received already, and the gathering process is started already
    kms_webrtc_session_agent_add_stored_ice_candidates (self, TRUE);
  }
    ...
    KmsIceCandidate * candidate)
{
  ...
  GST_DEBUG_OBJECT (self, "Add ICE remote candidate (%s)",
      kms_ice_candidate_get_candidate (candidate));
  ...
  // error_mode: false, because at this point the remote SDP might not have
  ...
  ret = kms_webrtc_session_agent_add_ice_candidate (self, candidate, FALSE);
    ...
    gboolean offerer)
{
  ...
  // error_mode: true, because at this point the remote SDP should have been
  // received already
  kms_webrtc_session_remote_sdp_add_stored_ice_candidates (self, TRUE);
