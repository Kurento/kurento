...
public class RemoteObjectTypedFactory {
	...
	public <E> E create(Class<E> clazz) {
		...
		return (E) Proxy.newProxyInstance(clazz.getClassLoader(),
				new Class[] { clazz }, new RemoteObjectInvocationHandler(remoteObject, factory));		
	}

	@SuppressWarnings("unchecked")
	public <F> F getFactory(Class<F> clazz) {				
		return (F) Proxy.newProxyInstance(clazz.getClassLoader(),
				new Class[] { clazz }, new FactoryInvocationHandler(clazz.getEnclosingClass(), factory));		
	}
