...
public class ProtocolManager {
	...
	private JsonRpcHandlerManager handlerManager;
	...
	/**
	 * Process incomming message. The response is sent using responseSender. If
	...
	private void processRequestMessage(ServerSessionFactory factory,
		...
		if (request.getMethod().equals(JsonRpcConstants.METHOD_RECONNECT)) {
	...
	private void processReconnectMessage(ServerSessionFactory factory,
		...
		if (sessionId == null) {
			...
			if (session != null) {
				...
				responseSender.sendResponse(new Response<Object>(sessionId,
						request.getId(),
						JsonRpcConstants.RECONNECTION_SUCCESSFUL));
			} else {
