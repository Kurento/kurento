...
public class CppObjectType implements TemplateMethodModelEx {
	...
	public Object exec(@SuppressWarnings("rawtypes") List arguments)
		...
		if (type instanceof TypeRef) {
			...
			if (typeRef.isList()) {
				if (isParam)
					return "const std::vector<" + getTypeAsString(
							typeRef.getName(), false, prefix, suffix) + "> &";
				else
					return "std::vector<" + getTypeAsString(typeRef.getName(),
							false, prefix, suffix) + ">";
			} else if (typeRef.isMap()) {
				if (isParam)
					return "const std::map <std::string," + getTypeAsString(
							typeRef.getName(), false, prefix, suffix) + "> &";
				else
					return "std::map <std::string," + getTypeAsString(
							typeRef.getName(), false, prefix, suffix) + ">";
			} else {
