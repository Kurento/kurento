...
public class Browser implements Closeable {
  ...
  public InputStream getExtensionAsInputStream(String extension) {
    ...
    try {
      ...
      log.warn("Exception reading extension {} in the classpath ({} : {})", extension,
  ...
  public void createSaucelabsDriver(DesiredCapabilities capabilities) throws MalformedURLException {
    ...
    driver =
        new RemoteWebDriver(new URL("http://" + sauceLabsUser + ":" + sauceLabsKey
            + "@ondemand.saucelabs.com:80/wd/hub"), capabilities);

    jobId = ((RemoteWebDriver) driver).getSessionId().toString();
    log.debug("%%%%%%%%%%%%% Saucelabs URL job for {} ({} {} in {}) %%%%%%%%%%%%%", id,
  ...
  public void injectKurentoTestJs() {
    if (this.getBrowserType() != BrowserType.IEXPLORER) {
      ...
      if (this.getProtocol() == Protocol.FILE) {
        File pageFile =
            new File(this.getClass().getClassLoader().getResource("static/lib/kurento-test.js")
                .getFile());
  ...
  public static class Builder {
    ...
    private Protocol protocol = Protocol.valueOf(getProperty(TEST_PROTOCOL_PROPERTY,
        TEST_PROTOCOL_DEFAULT).toUpperCase());
    private WebPageType webPageType = WebPageType.value2WebPageType(getProperty(TEST_PATH_PROPERTY,
        TEST_PATH_DEFAULT));
    ...
    public Builder scope(BrowserScope scope) {
      ...
      String appAutostart =
          getProperty(TestConfiguration.TEST_APP_AUTOSTART_PROPERTY,
