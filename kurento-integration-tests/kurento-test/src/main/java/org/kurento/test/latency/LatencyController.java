...
public class LatencyController implements ChangeColorEventListener<ChangeColorEvent> {
  ...
  public void checkLocalLatency(final long testTime, final TimeUnit testTimeUnit, WebPage client)
    ...
    while (true) {
      ...
      if (latency > getLatencyThreshold(TimeUnit.MILLISECONDS)) {
        ...
        client.takeScreeshot(KurentoClientBrowserTest.getDefaultOutputFile("-" + parsedtime
  ...
  public void checkLocalLatencyInBackground(final long testTime, final TimeUnit testTimeUnit,
  ...
  public void checkRemoteLatencyInBackground(final long testTime, final TimeUnit testTimeUnit,
      final WebPage localClient, final WebPage remoteClient) {
    new Thread() {
      public void run() {
        checkRemoteLatency(testTime, testTimeUnit, localClient, remoteClient);
      }
    }.start();
  }

  public void checkRemoteLatency(final long testTime, final TimeUnit testTimeUnit,
      WebPage localClient, WebPage remoteClient) {

    addChangeColorEventListener(new VideoTag(VideoTagType.LOCAL), localClient, getName() + " "
        + VideoTagType.LOCAL);
    addChangeColorEventListener(new VideoTag(VideoTagType.REMOTE), remoteClient, getName() + " "
        + VideoTagType.REMOTE);
    ...
    try {
      ...
      while (true) {
        ...
        log.info("latencyMilis={} -- lastLocalColor={} -- lastRemoteColor={} -- "
        ...
        if (similarColor(lastLocalColor, lastRemoteColor)) {
          ...
          if (latencyMilis > getLatencyThreshold(TimeUnit.MILLISECONDS)) {
            LatencyException latencyException =
                new LatencyException(latencyMilis, testTimeUnit, parsedLocaltime, parsedRemotetime,
  ...
  private boolean similarColor(Color expectedColor, Color realColor) {
    ...
    double distance =
        Math.sqrt((realRed - expectedRed) * (realRed - expectedRed) + (realGreen - expectedGreen)
            * (realGreen - expectedGreen) + (realBlue - expectedBlue) * (realBlue - expectedBlue));
