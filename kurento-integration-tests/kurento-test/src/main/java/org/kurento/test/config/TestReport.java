...
public class TestReport {
  ...
  public void initPage() {
    ...
    appendHtml("<script src='https://cdn.rawgit.com/eligrey/FileSaver.js/master/FileSaver.js'></script>");
  ...
  public String getCode(String text) {
    String code =
        "<button type='button' onclick=\"saveAs(new Blob([$(this).next().val()], "
    ...
    if (testScenario != null) {
      for (Browser bc : testScenario.getBrowserMap().values()) {
        if (bc.getScope() == BrowserScope.SAUCELABS) {
          ...
          appendHtml("<a href='https://saucelabs.com/tests/" + jobId + "'>" + bc.getId()
              + "</a><br>");
        }
      }
      if (saucelabsTitle) {
