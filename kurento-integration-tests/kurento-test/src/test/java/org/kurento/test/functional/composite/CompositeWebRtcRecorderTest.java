...
/**
 * Four synthetic videos are played by four WebRtcEndpoint (Connect only audio
 * for thres videos) and mixed by a Composite. The resulting video is recording
 * using a RecorderEndpoint. The recorded video is played using a
 * PlayerEndpoint.<br/>
 * 
 * Media Pipeline(s): <br>
 ...
 * 
 * Test logic: <br>
 * 1. (KMS) Media server mix the media from 4 WebRtcEndpoint and it records the
 * grid using the RecorderEndpoint. Only red color (video from first player) and
 * audio from four players.<br>
 * 2. (KMS) Media server implements a player to reproduce the video recorded in
 * step 1. <br>
...
public class CompositeWebRtcRecorderTest extends BaseRecorder {
	...
	public CompositeWebRtcRecorderTest(TestScenario testScenario) {
		super(testScenario);
	}
	...
	public void testCompositeRecorder() throws Exception {
		...
		launchBrowser(mp2, webRtcEP2, playerEP2, null,
				EXPECTED_VIDEO_CODEC_WEBM, EXPECTED_AUDIO_CODEC_WEBM,
				recordingFile, Color.RED, 0, 0, PLAYTIME);
