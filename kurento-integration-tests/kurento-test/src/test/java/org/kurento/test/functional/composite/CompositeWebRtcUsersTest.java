...
public class CompositeWebRtcUsersTest extends FunctionalTest {
  ...
  public static Collection<Object[]> data() {
    ...
    test.addBrowser(BROWSER5,
        new Browser.Builder().browserType(BrowserType.CHROME).webPageType(WebPageType.WEBRTC)
            .scope(BrowserScope.LOCAL).video(getTestFilesPath() + "/video/10sec/white.y4m").build());
  ...
  public void testCompositeWebRtcUsers() throws Exception {
    ...
    Assert.assertTrue("Upper right part of the video must be blue", getPage(BROWSER1)
        .similarColorAt(Color.BLUE, 450, 0));
    Assert.assertTrue("Lower left part of the video must be green", getPage(BROWSER1)
        .similarColorAt(Color.GREEN, 0, 450));
    Assert.assertTrue("Lower right part of the video must be white", getPage(BROWSER1)
