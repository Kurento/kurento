...
public class AlphaBlendingWebRtcTest extends FunctionalTest {
  ...
  public static Collection<Object[]> data() {
    ...
    test.addBrowser(BROWSER2,
        new Browser.Builder().browserType(BrowserType.CHROME).scope(BrowserScope.LOCAL)
            .webPageType(WebPageType.WEBRTC).video(getTestFilesPath() + "/video/10sec/green.y4m")
            .build());
    test.addBrowser(BROWSER3,
        new Browser.Builder().browserType(BrowserType.CHROME).scope(BrowserScope.LOCAL)
            .webPageType(WebPageType.WEBRTC).video(getTestFilesPath() + "/video/10sec/blue.y4m")
            .build());
  ...
  public void testAlphaBlendingWebRtc() throws Exception {
    ...
    // Assertions
    Assert.assertTrue("Upper left part of the video must be blue", getPage(BROWSER4)
        .similarColorAt(Color.GREEN, 0, 0));
    Assert.assertTrue("Lower right part of the video must be red", getPage(BROWSER4)
        .similarColorAt(Color.RED, 315, 235));
    ...
    Assert.assertTrue("Lower right part of the video must be blue", getPage(BROWSER4)
        .similarColorAt(Color.BLUE, 315, 235));
