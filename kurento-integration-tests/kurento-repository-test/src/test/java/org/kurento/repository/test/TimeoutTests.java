...
public class TimeoutTests extends BaseRepositoryTest {
  ...
  public void playerAutoTerminationTest() throws Exception {
    ...
    assertEquals(HttpStatus.OK, template.getForEntity(player.getURL(), byte[].class)
        .getStatusCode());
    ...
    assertEquals(HttpStatus.NOT_FOUND, template.getForEntity(player.getURL(), byte[].class)
        .getStatusCode());
