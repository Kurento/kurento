...
public class ParamsFlattener {
   ...
   * entry for each property with its name and value. The value of the property is also flatten.</li>
   * </ul>
   *
   * @param param
   * @return
  ...
  private boolean isPrimitiveClass(Class<?> clazz) {
    return clazz == String.class || clazz == Boolean.class || clazz == Float.class
        ...
        || clazz == int.class || clazz == void.class || clazz == Void.class
        || clazz == double.class || clazz == Double.class || clazz == long.class
        || clazz == Long.class;
  }
  ...
  private Object unflattedComplexType(Class<?> clazz, Props props, ObjectRefsManager manager) {
    ...
    for (int i = 0; i < constParams.length; i++) {
      ...
      constParams[i] =
          unflattenValue(paramName, constClasses[i], props.getProp(paramName), manager);
    }

    try {
      return constructor.newInstance(constParams);
    } catch (Exception e) {
      throw new ProtocolException("Exception while creating an object for the class '"
