...
public class ModuleClassesManager {
  ...
  private final ConcurrentHashMap<String, String> packageNamesByModuleName =
  ...
  public Class<?> getClassFor(String moduleName, String typeName) {
    ...
    try {
      ...
      throw new ProtocolException("Exception creating Java Class for '" + moduleName + "."
          + typeName + "'", e);
    }
  }
  ...
  private String getModuleInfoClassName(String moduleName) {
    ...
    String moduleNameWithFirstUpper =
        moduleName.substring(0, 1).toUpperCase() + moduleName.substring(1, moduleName.length());
