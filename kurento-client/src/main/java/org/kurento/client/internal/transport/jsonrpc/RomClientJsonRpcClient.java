...
public class RomClientJsonRpcClient implements RomClient {
	...
	public Object invoke(String objectRef, String operationName, Props operationParams, Type type,
		...
		RequestAndResponseType reqres = createInvokeRequest(objectRef, operationName,
				operationParams, type, false);
	...
	private <P, R> R sendRequest(Request<JsonObject> request, final Type responseType,
		...
		try {
			...
			if (cont == null) {
				...
				return processReqResult(responseType, processor, client
						.sendRequest(request.getMethod(), request.getParams(), JsonElement.class));
	...
	public RequestAndResponseType createSubscribeRequest(String objectRef, String eventType) {
		...
		JsonObject params = JsonUtils.toJsonObject(
				new Props(SUBSCRIBE_OBJECT, objectRef).add(SUBSCRIBE_TYPE, eventType));
	...
	public RequestAndResponseType createUnsubscribeRequest(String objectRef,
		...
		JsonObject params = JsonUtils.toJsonObject(new Props(UNSUBSCRIBE_OBJECT, objectRef)
	...
	public void transaction(final List<Operation> operations,
		...
		List<Response<JsonElement>> responses = this.sendRequest(
				new Request<>(TRANSACTION_METHOD, params),
	...
	private void processTransactionResponse(List<Operation> operations,
		...
		if (e != null) {
			...
			for (int i = 0; i < operations.size(); i++) {
				...
				op.processResponse(
						processReqResult(reqres.responseType, null, response.getResult()));
			}
		}
	}
