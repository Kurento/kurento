...
public class RomServerJsonRpcHandler extends DefaultJsonRpcHandler<JsonObject> {
    ...
    try {
      ...
      switch (method) {
        case INVOKE_METHOD:
          ...
          break;
        case RELEASE_METHOD:
          String objectReleaseRef =
              getAsString(params, RELEASE_OBJECT, "object reference to be released");
  ...
  private String getAsString(JsonObject jsonObject, String propName, String propertyDescription) {
    ...
    if (element == null) {
      throw new ProtocolException("It is necessary a property '" + propName + "' with "
          + propertyDescription);
    } else {
  ...
  private void handleInvokeCommand(Transaction transaction, String objectRef, String operationName,
    ...
    Object result =
        server.invoke(objectRef, operationName, JsonUtils.fromJson(operationParams, Props.class),
            Object.class);
