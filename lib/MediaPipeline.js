...
function MediaPipeline(objectRef, parent)
{
  ...
  function genParams(type, params)
  {
    var params2 =
    {
      type: type
 ...
 /**
  ...
  this.createMediaElement = function(type, params, callback)
  {
    ...
    params = genParams(type, params);

    this.emit('request', 'createMediaElement', params, function(error, objectRef)
    {
      if(error)
      {
        if(callback)
  ...
  this.createMediaMixer = function(type, params, callback)
  {
    ...
    this.emit('request', 'createMediaMixer', params, function(error, objectRef)
    {
      if(error)
      {
        if(callback)
