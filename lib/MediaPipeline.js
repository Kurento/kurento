...
var elements = require('./endpoints');
...
function MediaPipeline(objectRef, parent)
{
 ...
 /**
  ...
  this.createMediaElement = function(type, params, callback)
  {
    ...
    {
      self._rpc('createMediaElement', 'pipeline', item,
      function(error, objectRef)
      {
        if(error) return callback(error);

        // If element type is not registered, use generic MediaElement
        var constructor = elements[item.type] || MediaElement;

        var params = item.params || {};

        var mediaElement = new constructor(objectRef, self, self, params);
    ...
    if(type instanceof Array)
    {
      ...
      for(var i=0, t; t=type[i]; i++)
      {
        ...
        if(param)
        {
          // If element type is not registered, use generic MediaElement
          var constructor = elements[item.type] || MediaElement;
          item.params = constructor.checkparams(param);
        }
    ...
    else
    {
      ...
      if(params)
      {
        // If element type is not registered, use generic MediaElement
        var constructor = elements[item.type] || MediaElement;
        item.params = constructor.checkparams(params);
      }
  ...
  this.createMediaMixer = function(type, params, callback)
  {
    ...
    this._rpc('createMediaMixer', 'pipeline', params2, function(error, objectRef)
    {
      if(error) return callback(error);
...
/**
 * @see {@link module:KwsMedia~MediaContainer.checkparams}
 */
MediaPipeline.checkparams = MediaContainer.checkparams;
