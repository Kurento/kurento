...
public class RepositoryHttpServlet extends HttpServlet {
  ...
  protected static class Range {
    ...
    public boolean validate() {
      ...
      return (start >= 0) && (end >= 0) && (start <= end) && (length == -1 || length > 0);
    }
  }
  ...
  private String configureServletMapping(ServletConfig servletConfig) {
    Collection<String> mappings =
        servletConfig.getServletContext().getServletRegistration(servletConfig.getServletName())
            .getMappings();
      ...
      if (range != null) {
        ...
        if (range.start > elem.getWrittenBytes()) {
          ...
          resp.getOutputStream().println(
              "The server doesn't support writing ranges " + "ahead of previously written bytes");
   ...
   * same resource.
   */
  protected File executePartialPut(HttpServletRequest req, Range range, String sessionId)
      throws IOException {
    ...
    try (RandomAccessFile randAccessContentFile = new RandomAccessFile(contentFile, "rw")) {
      ...
      RepositoryHttpEndpointImpl repoItemHttpElem = repoHttpManager.getHttpRepoItemElem(sessionId);

      // Copy data in oldRevisionContent to contentFile
      if (repoItemHttpElem != null) {

        try (BufferedInputStream bufOldRevStream =
            new BufferedInputStream(repoItemHttpElem.createRepoItemInputStream(), FILE_BUFFER_SIZE)) {
      ...
      try (BufferedInputStream requestBufInStream =
          new BufferedInputStream(req.getInputStream(), FILE_BUFFER_SIZE)) {
  ...
  protected void serveResource(HttpServletRequest request, HttpServletResponse response,
    ...
    if (malformedRequest || (noRanges && request.getHeader("Range") == null) || ranges == FULL) {
      ...
      if (ranges.size() == 1) {
        ...
        response.addHeader("Content-Range", "bytes " + range.start + "-" + range.end + "/"
            + range.length);
  ...
  private String getContentType(RepositoryHttpEndpointImpl repoItemHttpElem,
    ...
    if (contentType == null) {
      contentType = getServletContext().getMimeType(repoItemHttpElem.getRepositoryItem().getId());
  ...
  protected boolean checkIfUnmodifiedSince(HttpServletRequest request,
      HttpServletResponse response, RepositoryItemAttributes resourceAttributes) throws IOException {
  ...
  protected void copy(RepositoryHttpEndpointImpl repoItemHttpElem, HttpServletResponse response,
    ...
    try (ServletOutputStream ostream = response.getOutputStream()) {
      try (InputStream istream =
          new BufferedInputStream(repoItemHttpElem.createRepoItemInputStream(), INPUT_BUFFER_SIZE)) {
