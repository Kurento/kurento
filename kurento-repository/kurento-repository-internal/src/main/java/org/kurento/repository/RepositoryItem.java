...
public interface RepositoryItem {
   ...
   * using the provided URL, the state of the item is changed to
   * {@link RepositoryItem.State#STORING STORING}.
   * 
   ...
   * Returns the {@link RepositoryHttpPlayer} to download the contents of the item using http
   * protocol. This operation is only valid when the item is in {@link RepositoryItem.State#STORED
   * STORED} state. The parameter {@code sessionIdInURL} allows to specify the sessionId of this
   * player used to construct the URL. The complete URL of the player can be obtained using the
   ...
   * protocol. This operation is only valid when the item is in NEW state. When the element is used
   * using the provided URL, the state of the item is changed to
   * {@link RepositoryItem.State#STORING STORING}. The parameter {@code sessionIdInURL} allows to
   * specify the sessionId of this recorder used to construct the URL. The complete URL of the
   * recorder can be obtained using the {@link RepositoryHttpRecorder#getURL()} in the returned
   * object.
   * 
   * @param sessionIdInURL
   *          The sessionId of this player used to construct the URL.
